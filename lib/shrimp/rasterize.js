...
function renderUrl(url, output, options) {
  ...
  page.open(url, function (status) {
    if (status !== 'success' || (statusCode != 200 && statusCode != null)) {
      ...
      // Extracted from
      // https://github.com/ariya/phantomjs/blob/master/examples/printheaderfooter.coffee
      ...
      */
      if (page.evaluate(function(){return typeof ShrimpPdfObject == "object";})) {
        paperSize = page.paperSize;
        paperSize.header = paperSize.header || {}
        paperSize.footer = paperSize.footer || {}

        paperSize.header.height = page.evaluate(function() {
          return ShrimpPdfObject.header.height;
        });
        paperSize.header.contents = phantom.callback(function(pageNum, numPages) {
          return page.evaluate(function(pageNum, numPages){return ShrimpPdfObject.header.contents(pageNum, numPages);}, pageNum, numPages);
        });
        paperSize.footer.height = page.evaluate(function() {
          return ShrimpPdfObject.footer.height;
        });
        paperSize.footer.contents = phantom.callback(function(pageNum, numPages) {
          return page.evaluate(function(pageNum, numPages){return ShrimpPdfObject.footer.contents(pageNum, numPages);}, pageNum, numPages);
        });
